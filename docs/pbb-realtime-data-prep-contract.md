...
- prepare the PBB Support Realtime client profile
- prepare Support policies for server and admin/operator collaboration
- prepare Support project scopes for server and admin/operator workspaces
- apply Maestro telemetry settings generated by Kit
- verify packaged PBB client records and Maestro telemetry settings without exposing secrets
...
The packaged default source contains Hotline and Support reference records.
...
## Support Data Prep Handoff

Support must use its own Realtime client/project scope instead of reusing Hotline codes:

- `client_code`: `clt_PBB_SUPPORT`
- server project: `prj_SUPPORT_SERVER`
- admin/operator project: `prj_SUPPORT_ADMIN`

Kit should pass these values to Support Data Prep using the Support-owned keys already accepted by Support:
...
```json
{
  "shared": {
    "secrets": {
      "values": {
        "support_realtime_client_code": "clt_PBB_SUPPORT",
        "support_realtime_server_project_code": "prj_SUPPORT_SERVER",
        "support_realtime_admin_project_code": "prj_SUPPORT_ADMIN",
        "support_realtime_backend_ingress_secret": "<generated Support-specific secret>"
      }
    }
  }
}
...
`support_realtime_backend_ingress_secret` should be generated separately for Support. Do not intentionally reuse the generic `realtime_backend_ingress_secret` or Hotline ingress secret for new Support installs. Support currently accepts `shared.secrets.values.realtime_backend_ingress_secret` as a compatibility fallback, but the Realtime contract for Kit-managed installs is a Support-specific secret.

To register the Support backend ingress secret in Realtime, Kit can either inject `backend_ingress_secret` into the `clt_PBB_SUPPORT` record before running `tools/populate-initial-data.php`, or append a minimal `realtime.populate.clients[]` overlay with the same `client_code`, `name`, and `backend_ingress_secret`. Realtime stores only a hash/digest; reports must never expose the raw secret. If an existing Support client already has a backend ingress secret, Realtime preserves it unless `realtime.populate.options.overwrite_secrets=true` is explicitly supplied.
...
- packaged clients exist
- packaged policies exist
- packaged project scopes exist
